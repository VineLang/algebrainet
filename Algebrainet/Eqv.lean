...
inductive EqvA (S : System) : {i₁ o₁ i₂ o₂ : Nat} -> (h : (i₁ = i₂) ∧ (o₁ = o₂)) -> (x : Net S i₁ o₁) -> (y : Net S i₂ o₂) -> Prop
  ...
  | cat_assoc :
    ...
    {zᵢ zₒ : Nat} -> {z : Net S zᵢ zₒ} ->
    {h₀ h₁ h₂ h₃ : _} ->
    EqvA S # (cat h₀ x (cat h₁ y z)) (cat h₂ (cat h₃ x y) z)
  ...
  | move_swap : EqvA S # (cat # (cat # (mix wire swap) (mix swap wire)) (mix wire swap)) (cat # (cat # (mix swap wire) (mix wire swap)) (mix swap wire))
